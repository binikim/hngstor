...
export const Footer = () => {
  ...
  return (
    <footer className="bg-[#0a0a0a] pt-20 pb-10 px-6 md:px-12">
      <div className="max-w-[1920px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mb-20">
          <div className="space-y-6">
            <Link to="/" className="text-2xl font-headline font-extrabold text-primary tracking-tighter">
              {footerInfo?.logoText || 'H&G스토아'}
            </Link>
            <p 
              className="text-white/70 text-sm leading-relaxed max-w-xs"
              dangerouslySetInnerHTML={{ __html: footerInfo?.copyrightText || '© 2024 H&G스토아. 본 사이트는 만 19세 미만의 청소년의 출입을 금합니다. 성인 인증 후 모든 콘텐츠 이용이 가능합니다.' }}
            />
          </div>

          <div className="grid grid-cols-2 gap-8 lg:col-span-2">
            <div className="space-y-6">
              <h4 className="text-white font-headline font-bold text-sm uppercase tracking-widest">정보</h4>
            ...
            <div className="space-y-6">
              <h4 className="text-white font-headline font-bold text-sm uppercase tracking-widest">고객센터</h4>
              <ul className="space-y-3 text-sm text-white/70">
        ...
        <div className="pt-8 border-t border-white/10 text-center">
          <p className="text-[10px] text-white/70 uppercase tracking-[0.2em] leading-relaxed">
            {Array.isArray(footerInfo) 
              ? footerInfo.filter(item => item.label || item.value).map(item => `${item.label}: ${item.value}`).join(' | ')
              : <>
                  사업자등록번호: {footerInfo?.businessNumber || '000-00-00000'} | 대표: {footerInfo?.ceo || 'H&G 팀'} | 전화: {footerInfo?.phone || '00-000-0000'} | 주소: {footerInfo?.address || '서울특별시'}
